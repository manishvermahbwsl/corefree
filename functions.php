...
function cyberchimps_default_site_title() {
	...
	if ( is_archive() ) {
		echo ' | ';
		if ( is_category() ) {
			printf( esc_html( 'Category Archives: %s', 'cyberchimps_core' ), single_cat_title( '', false ) );
		} elseif ( is_tag() ) {
			printf( esc_html( 'Tag Archives: %s', 'cyberchimps_core' ), single_tag_title( '', false ) );
		} elseif ( is_author() ) {
			esc_html_e( 'Author Archives', 'cyberchimps_core' );
		} elseif ( is_day() ) {
			printf( esc_html( 'Daily Archives: %s', 'cyberchimps_core' ), get_the_date() );
		} elseif ( is_month() ) {
			printf( esc_html( 'Monthly Archives: %s', 'cyberchimps_core' ), get_the_date( 'F Y' ) );
		} elseif ( is_year() ) {
			printf( esc_html( 'Yearly Archives: %s', 'cyberchimps_core' ), get_the_date( 'Y' ) );
		} elseif ( is_plugin_active( 'woocommerce/woocommerce.php' ) && is_woocommerce() && is_shop() ) {
...
function cyberchimps_upgrade_bar() {
	...
	<div class="upgrade-callout">
		<p><img src="<?php echo get_template_directory_uri(); ?>/cyberchimps/options/lib/images/chimp.png" alt="CyberChimps"/>
			<?php
			printf(
				esc_html( 'Welcome to %1$s! Learn more now about upgrading to %2$s today.', 'cyberchimps_core' ),
				$current_theme_name,
				'<a href="' . $upgrade_link . '" target="_blank" title="' . $pro_title . '">' . $pro_title . '</a> '
			);
			?>
